...
export async function buildCastleShell(scene, collision, meta = null) {
  const scan = await buildScanCastle(scene, collision, meta);   // whole=整体扫描城堡 / graft=门面目 / null=纯 Kenney
...
export async function buildKenneyShell(scene, collision, opts = {}) {
  ...
  if (skipSC) {
    const seamMat = new THREE.MeshLambertMaterial({ color: 0x75808a });   // 过渡色（Kenney 石材 0x8a9096 与 graft tint 之间，不再强调分界）
