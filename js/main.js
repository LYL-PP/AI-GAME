...
// 分阶段加载（2026-07）：boot 只到标题页；点开始/继续后按阶段装载
//   阶段 A 户外（岛屿/海面/城堡/码头道具）→ 先放行可玩；
//   阶段 B 室内（大厅扫描件/家具/瓷人/10 宾客）→ 后台流式，门口拦截"整理室内……"；
//   阶段 C 其余（终局道具复用 A 资产，空载占位）。
...
import { buildVilla, buildVillaShell } from './world/villa.js';
...
// 各场景道具体积估算（MB，2026-07 实测；加载页进度分母）
const PROP_EST_MB = {
  jetty: 3.8, boat: 9.4, rock1: 4.3, rock2: 2.8, rock3: 6.3, tree: 3.6, cloud: 4.8, grass: 18,
  castle: 22, hall: 21, gramophone: 4.4, figurine: 9.6, dining_table: 3.6, dining_chair: 4.0,
  candelabra: 5.2, chandelier: 7.7, rug: 5.3, sofa: 8.5,
};
...
async function boot() {
  ...
  // ---------- 加载页 / 进度 ----------
  const LD = data.ui.system.loading || { stageA: '', stageB: '', stageC: '', gateWait: '整理室内……', items: {} };
  const itemName = (k) => LD.items?.[k] || k;
  const itemMeta = (k) => ({ label: itemName(k), est: (PROP_EST_MB[k] || 5) * 1048576 });
  const elLoad = document.getElementById('loadScreen');
  const elBar = document.getElementById('loadBar');
  const elPct = document.getElementById('loadPct');
  const elItem = document.getElementById('loadItem');
  const elStage = document.getElementById('loadStage');
  const elGate = document.getElementById('gateWait');
  const setStageText = (t) => { elStage.textContent = t || ''; };
  LoadTracker.onUpdate((s) => {
    const pct = s.total > 0 ? Math.min(99, Math.round((s.loaded / s.total) * 100)) : 0;
    elBar.style.width = pct + '%';
    elPct.textContent = pct + '%';
    elItem.textContent = s.current ? '正在载入：' + s.current : ' ';
    elGate.textContent = LD.gateWait + ' ' + pct + '%';
  });

  // ---------- 标题画面（数据就绪即出；3D 资产点开始后才分阶段装载） ----------
  ...
  document.getElementById('loadTitle').textContent = sys.gameTitleMain || '无人生还';
  ...
  // ---------- 阶段运行态 ----------
  let island = null, weather = null, player = null;
  let villaShell = null;    // 阶段 A 极简别墅外壳（B 建成后移除）
  let bDone = false;          // 阶段 B 完成（全系统就位）
  let fullTick = null;        // 完整帧逻辑（B 完成后接管）

  // 阶段 B 未完时禁止进入别墅轮廓（防"建成中人"）：最近边推出 + 近门提示（含进度）
  function gateVilla() {
    const f = player.feet;
    const inVilla = f.x > -12.6 && f.x < 12.6 && f.z > -8.6 && f.z < 8.8;
    if (inVilla) {
      const dxl = f.x + 12.6, dxr = 12.6 - f.x, dzb = f.z + 8.6, dzf = 8.8 - f.z;
      const m = Math.min(dxl, dxr, dzb, dzf);
      if (m === dxl) f.x = -12.6;
      else if (m === dxr) f.x = 12.6;
      else if (m === dzb) f.z = -8.6;
      else f.z = 8.8;
    }
    const near = f.x > -13.6 && f.x < 13.6 && f.z > -9.6 && f.z < 9.8;
    const show = near ? 'block' : 'none';
    if (elGate.style.display !== show) elGate.style.display = show;
  }

  // ---------- 阶段 A：户外（岛屿/海面/城堡/码头道具 → 可玩） ----------
  async function stageA() {
    setStageText(LD.stageA);
    ...
    }, Object.fromEntries(['jetty', 'rock1', 'rock2', 'rock3', 'tree', 'cloud', 'boat', 'grass'].map((k) => [k, itemMeta(k)])));
    island = buildIsland(scene, collision, data);
    collision.setGroundFunction(island.groundHeight);
    // 城堡外壳（扫描级 castle_clean.glb；失败回退 Kenney / 程序化）
    let castleRefs = null;
    try {
      castleRefs = await buildCastleShell(scene, collision, itemMeta('castle'));
    } catch (e) {
      console.warn('[castle] 外壳构建失败，使用程序化外墙', e);
    }
    // 天气（室内灯位由阶段 B 建成别墅后注入）
    weather = new Weather(scene, renderer, {
      sun, hemi,
      seaMat: island.seaMat,
      seaUniforms: island.seaUniforms,
      indoor: { ceiling: [], fire: { intensity: 0, userData: { base: 0 } }, candles: [] },
      flames: [], candleFlames: [],
      windowGlow: castleRefs?.windowGlow,
      clouds: island.clouds,
      boat: island.boat,
    });
    weather.setChapter(startChapter);
    // 极简别墅外壳（保持码头第一帧构图；B 建成后移除）
    villaShell = buildVillaShell(scene);
    // 玩家（A 完成即放行）
    player = new Player(camera, collision, renderer.domElement);
  ...
  // ---------- 阶段 B：室内（大厅扫描件/别墅家具/瓷人/10 宾客 + 全系统接线） ----------
  async function stageB() {
    setStageText(LD.stageB);
    const { preloadSceneProps } = await import('./world/sceneProps.js');
    await preloadSceneProps({
    ...
    }, Object.fromEntries(['gramophone', 'figurine', 'dining_table', 'dining_chair', 'candelabra', 'chandelier', 'rug', 'sofa'].map((k) => [k, itemMeta(k)])));
    // 扫描大厅：先探载 hall.glb（失败则 villa 按原样构建程序化大厅）
    const { preloadHallScan, buildHallScan } = await import('./world/hallScan.js');
    const hallScanOk = await preloadHallScan(itemMeta('hall'));
    const villa = buildVilla(scene, collision, data, { hallScan: hallScanOk });
    if (villaShell) { scene.remove(villaShell); villaShell = null; }   // 真别墅就位，撤极简外壳
    const hallScanGroup = hallScanOk ? buildHallScan(scene) : null;
    // 室内灯位注入天气系统
    weather.refs.indoor = villa.indoor;
    weather.refs.flames = villa.flames;
    weather.refs.candleFlames = villa.candleFlames;

    // ---------- NPC / 日程 / 对话 / 序章 ----------
    const npcManager = await NPCManager.create(scene, data.characters, true);
    ...
    // 章节初始态（同原 boot 顺序：先 setChapter(startChapter)，后按需 applySave）
    if (explicitChapter || !hasSave) setChapter(startChapter);
    if ((playMode || startOverlay.dataset.cont === '1') && hasSave && params.get('fresh') !== '1') applySave();
    ...
    bDone = true;
    window.__ready = true;   // 全系统就位（自动验收轮询点）
  }

  // ---------- 阶段 C：其余（终局演出复用 A 资产，无独占 GLB，空载占位） ----------
  async function stageC() {
    setStageText(LD.stageC);
  }

  // ---------- 主循环（A 完成后启动；B 未完为户外精简循环） ----------
  function startLoop() {
    ...
    renderer.setAnimationLoop(() => {
      ...
      if (!bDone) {
        ...
        camera.position.set(player.feet.x, player.feet.y + 1.62, player.feet.z);
        camera.rotation.set(player.pitch, player.yaw, 0);
        weather.update(dt, camera.position, false);
        gateVilla();
        renderer.render(scene, camera);
        return;
      }
      if (fullTick) fullTick(dt);
    });
  }
  ...
  async function startGame(mode) {
    ...
    if (mode === 'continue') startOverlay.dataset.cont = '1';
    await stageA();
    startLoop();
    if (mode === 'fresh') {
      // A 完成即放行可玩；B/C 后台流式
      ...
      stageB().then(stageC).catch((e) => console.error('[stageB] 加载失败：', e));
    } else {
      await stageB();
      await stageC();
      elLoad.style.display = 'none';
      if (!playMode) startOverlay.style.display = 'flex';
    }
  }
  ...
  if (!playMode) {
    ...
    titleEl.style.display = 'none';
    startGame('auto');   // 调试模式：全量加载对应阶段后直接进游戏
  }
