...
async function main() {
  ...
  for (const v of ['front', 'back', 'left', 'right']) {
    await send('Page.navigate', { url: `http://localhost:8000/tools/scene_probe.html?m=dining_chair&v=${v}` });
    for (let i = 0; i < 45; i++) { await sleep(500); if (await evaljs('!!window.__ready')) break; }
    const s = await send('Page.captureScreenshot', { format: 'png' });
    writeFileSync(join(ROOT, 'docs/screenshots', `chair_${v}.png`), Buffer.from(s.data, 'base64'));
    console.log('SHOT:', `chair_${v}.png`);
  }
