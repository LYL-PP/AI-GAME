...
async function main() {
  ...
  for (const [name, seat] of jobs) {
    await evaljs(`(() => {
      ...
      const n = NPCAPI.get('${name}');
      const hy = n.rigged ? 1.8 + 1.28 : 1.8 + 0.87 * (n.spec.h || 1.75) - 0.42;
      const cx = sx + (T.x - sx) * 0.3, cz = sz + (T.z - sz) * 0.3;
      const yaw = Math.atan2(-(sx - cx), -(sz - cz));
      DebugAPI.teleport(cx, cz, yaw, hy + 0.05 - 1.62);   // player eye = feet+1.62 → 让视高等效 hy+0.05
    })()`);
    await sleep(400);
    const s = await send('Page.captureScreenshot', { format: 'png' });
    writeFileSync(join(ROOT, 'docs/screenshots', `static_${name}.png`), Buffer.from(s.data, 'base64'));
    console.log('SHOT:', `static_${name}.png`);
  }
