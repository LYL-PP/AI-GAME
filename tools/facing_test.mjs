...
async function main() {
  ...
  console.log(await evaljs(`(() => {
    ...
    return JSON.stringify({ pos: v.pos.toArray().map(x=>+x.toFixed(2)), yaw: +v.yaw.toFixed(2), rotY: +v.group.rotation.y.toFixed(2) });
  })()`));
  // 机位 A：-(sin,cos) 前向 2m 回看（若面向桌心，此为她脸）
  await evaljs(`(() => {
    const v = NPCAPI.get('vera');
    const fx = -Math.sin(v.yaw), fz = -Math.cos(v.yaw);
    DebugAPI.teleport(v.pos.x + fx * 2.0, v.pos.z + fz * 2.0, v.yaw, v.pos.y);
  })()`);
  await sleep(400);
  await shot('facing_A.png');
  // 机位 B：反向 +(sin,cos) 2m 回看
  await evaljs(`(() => {
    const v = NPCAPI.get('vera');
    const fx = Math.sin(v.yaw), fz = Math.cos(v.yaw);
    DebugAPI.teleport(v.pos.x + fx * 2.0, v.pos.z + fz * 2.0, v.yaw + Math.PI, v.pos.y);
  })()`);
  await sleep(400);
  await shot('facing_B.png');
