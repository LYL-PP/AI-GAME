...
async function main() {
  ...
  await sleep(16000);
  await evaljs('DebugAPI.teleport(-5.0, 2.8, 0, 1.8)');
  await sleep(400);
  let s = await send('Page.captureScreenshot', { format: 'png' });
  writeFileSync(join(ROOT, 'docs/screenshots', 'p0_mantel_far.png'), Buffer.from(s.data, 'base64'));
  console.log('SHOT: p0_mantel_far.png');
