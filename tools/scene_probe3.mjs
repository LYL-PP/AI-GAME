...
async function main() {
  ...
  for (const v of ['c_posz', 'c_negz', 'c_posx', 'c_negx']) {
    await send('Page.navigate', { url: `http://localhost:8000/tools/scene_probe.html?m=hall&v=${v}` });
    for (let i = 0; i < 45; i++) { await sleep(500); if (await evaljs('!!window.__ready')) break; }
    const s = await send('Page.captureScreenshot', { format: 'png' });
    writeFileSync(join(ROOT, 'docs/screenshots', `scan3_hall_${v}.png`), Buffer.from(s.data, 'base64'));
    console.log('SHOT:', `scan3_hall_${v}.png`);
  }
