...
async function main() {
  ...
  await sleep(16000);
  await evaljs('DebugAPI.teleport(-5.0, 1.2, 0, 1.8)');
  await sleep(300);
  await shot('scan_on_mantel.png');
  await evaljs(`(() => {
    const scene = NPCAPI.get('vera').group.parent;
    const g = scene.getObjectByName('hallScan');
    if (g) g.visible = false;
    // 同时强制保持隐藏（主循环每帧会重置）
    window.__forceHide = setInterval(() => { const gg = scene.getObjectByName('hallScan'); if (gg) gg.visible = false; }, 200);
  })()`);
  await sleep(600);
  await shot('scan_off_mantel.png');
