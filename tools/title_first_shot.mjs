...
async function main() {
  ...
  await sleep(2500);
  await shot('title_first_2s.png');
  await sleep(12000);
  await shot('title_ready.png');
  const r = await send('Runtime.evaluate', { expression: `JSON.stringify({loading: getComputedStyle(document.getElementById('titleLoading')).display, btns: getComputedStyle(document.querySelector('#titleScreen .title-btns')).display, startOv: getComputedStyle(document.getElementById('startOverlay')).display})`, returnByValue: true });
  console.log('就绪状态:', r?.result?.value);
