...
async function main() {
  ...
  for (const m of ['gramophone', 'dining_chair', 'dining_table', 'figurine']) {
    for (const v of ['iso', 'front']) {
      ...
      writeFileSync(join(ROOT, 'docs/screenshots', `prop_${m}_${v}.png`), Buffer.from(s.data, 'base64'));
      console.log('SHOT:', `prop_${m}_${v}.png`);
    }
  }
