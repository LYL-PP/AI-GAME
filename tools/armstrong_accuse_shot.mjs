// armstrong_accuse_shot.mjs —— 阿姆斯特朗指控运镜帧（accuseName 命中即拍）+ 布伦特毛线团/坐姿键自检
...
const PORT = 9252;
const edge = spawn(EDGE, ['--headless=new','--disable-gpu','--enable-unsafe-swiftshader',`--remote-debugging-port=${PORT}`,`--user-data-dir=${join(ROOT,'tools/.edge-profile-arm')}`,'--window-size=1280,720','about:blank'], { stdio: 'ignore', cwd: ROOT });
...
async function main() {
  ...
  await sleep(1500);
  // 自检：布伦特毛线团挂点 + 坐姿键（ch5 冻结特判依赖 rigCfg.sit）
  console.log('brent items:', await evaljs('Object.keys(NPCAPI.get("brent")?.rigged?.items || {}).join(",")'),
    '| knitProp:', await evaljs('!!NPCAPI.get("brent")?.knitProp'),
    '| sit键:', await evaljs('NPCAPI.get("brent")?.rigCfg?.sit'));
  console.log('armstrong dying:', await evaljs('NPCAPI.get("armstrong")?.rigged?.has("dying")'));
  // 指控运镜帧：阿姆斯特朗 charge
  ...
  for (let i = 0; i < 90 && !hit; i++) {
    ...
    if (t && t.includes('阿姆斯特朗')) hit = true;
    else { await evaljs('PrologueAPI.skip()'); await sleep(350); }
  }
  console.log('charge 阿姆斯特朗', hit ? 'hit' : 'MISS', 'name=', await evaljs(accuseName));
  await sleep(600);
  await shot('final_accuse_armstrong.png');
