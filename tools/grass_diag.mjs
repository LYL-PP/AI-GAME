...
async function main() {
  ...
  console.log(await evaljs(`(async () => {
    ...
    const out = [];
    scene.traverse((o) => {
      if (o.isInstancedMesh && o.count <= 20) {
        const bb = new T.Box3().setFromObject(o);
        out.push({ count: o.count, idx: o.geometry.index ? o.geometry.index.count : -1, bbSize: bb.getSize(new T.Vector3()).toArray().map(v=>+v.toFixed(1)) });
      }
    });
