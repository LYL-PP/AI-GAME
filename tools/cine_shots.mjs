// cine_shots.mjs —— 指控电影期间按字幕内容截取被指控人特写
...
async function main() {
  ...
  await sleep(2600); // 等 cine 开始
  await evaljs('WeatherAPI.setChapter(0)'); // 撤销骤暗（仅为截图取证，不影响流程）
  // 轮询字幕，命中即拍（每目标只在首次命中后 1.2s（相机到位）拍一张）
  const targets = [
    { key: '马尔斯顿', file: 'cine_marston.png' },
    { key: '隆巴德', file: 'cine_lombard.png' },
    { key: '维拉', file: 'cine_vera.png' },
  ];
  const done = new Set();
  for (let i = 0; i < 400 && done.size < targets.length; i++) {
