...
async function main() {
  ...
  await evaljs(`(() => {
    ...
    DebugAPI.teleport(v.pos.x + fx * 1.6, v.pos.z + fz * 1.6, v.yaw, v.pos.y - 0.35);
  })()`);
  await sleep(400);
  const s = await send('Page.captureScreenshot', { format: 'png' });
  writeFileSync(join(ROOT, 'docs/screenshots', 'vera_face_final.png'), Buffer.from(s.data, 'base64'));
  console.log('SHOT: vera_face_final.png');
